...
import React, { useState, useEffect } from 'react';
import { Search, Leaf, Thermometer, Droplets, Sun } from 'lucide-react';
import { motion } from 'framer-motion';
import { Link } from 'react-router-dom';
import api from '../services/api';
import LoadingSpinner from '../components/UI/LoadingSpinner';
...
const TerraBook = () => {
  ...
  // UPDATED: This useEffect hook now triggers a search when the user types.
  // A 500ms debounce is added to prevent API calls on every keystroke.
  useEffect(() => {
  ...
  }, [searchTerm, selectedCategory]); // Now correctly depends on searchTerm
  ...
  // UPDATED: This function no longer filters the list locally.
  // It now sets the search term and allows the main useEffect hook to trigger a proper API search.
  const handleSuggestionClick = (plant: Plant) => {
    setSearchTerm(plant.name);
    setShowSuggestions(false);
  };
        ...
        {isLoading ? (
          ...
          <>
            {plants.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {plants.map((plant, index) => (
                        <motion.div key={plant._id} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: index * 0.1 }}>
                            <Link to={`/plant/${plant._id}`}>
                                <div className="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-all transform hover:-translate-y-1 group">
                                    <div className="relative h-48 overflow-hidden">
                                        <img src={plant.images[0] || 'https://images.pexels.com/photos/1301856/pexels-photo-1301856.jpeg'} alt={plant.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                                        <div className="absolute top-4 right-4 bg-green-600 text-white px-3 py-1 rounded-full text-sm font-medium">{plant.category}</div>
                                    </div>
                                    <div className="p-6">
                                        <h3 className="text-xl font-bold text-gray-900 mb-2">{plant.name}</h3>
                                        <p className="text-sm text-gray-500 italic mb-3">{plant.scientificName}</p>
                                        <p className="text-gray-600 mb-4 line-clamp-3">{plant.description}</p>
                                        <div className="grid grid-cols-2 gap-4 mb-4">
                                            <div className="flex items-center space-x-2"><Thermometer className="h-4 w-4 text-red-500" /><span className="text-sm text-gray-600">{plant.weatherRequirements.temperature.min}-{plant.weatherRequirements.temperature.max}{plant.weatherRequirements.temperature.unit}</span></div>
                                            <div className="flex items-center space-x-2"><Sun className="h-4 w-4 text-yellow-500" /><span className="text-sm text-gray-600">{plant.weatherRequirements.sunlight}</span></div>
                                            <div className="flex items-center space-x-2"><Droplets className="h-4 w-4 text-blue-500" /><span className="text-sm text-gray-600">{plant.growthInfo.wateringFrequency}</span></div>
                                            <div className="flex items-center space-x-2"><Leaf className="h-4 w-4 text-green-500" /><span className="text-sm text-gray-600">{plant.growthInfo.harvestTime}</span></div>
                                        </div>
                                        <div className="flex items-center justify-between">
                                            <div className="flex flex-wrap gap-1">
                                                {plant.weatherRequirements.season.slice(0, 2).map(season => (<span key={season} className="bg-green-100 text-green-800 px-2 py-1 rounded-full text-xs">{season}</span>))}
                                            </div>
                                            <span className="text-green-600 font-medium text-sm group-hover:text-green-700">Learn More →</span>
                                        </div>
                                    </div>
                                </div>
                            </Link>
                ...
                <div className="text-center py-12">
                    <Leaf className="h-16 w-16 text-gray-300 mx-auto mb-4" />
                    <p className="text-gray-500 text-lg">No plants found matching your criteria.</p>
                    <button onClick={() => { setSearchTerm(''); setSelectedCategory(''); }} className="mt-4 text-green-600 hover:text-green-700 font-medium">Clear filters and show all plants</button>
                </div>
            )}
          </>
        )}
